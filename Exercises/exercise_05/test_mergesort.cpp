...
{ 
	...
	for (int i=0; i < 100; i++) {
		...
		mergesort(vec, 0, (int)vec.size()-1);		//metto int per evitare che se si genera un vettore di dimensione 0, si vada a ricadere in un -1 che vada in un numero enorme (size_t è unsigned!)
	...
	mergesort(costellazioni, 0, costellazioni.size()-1);
	...
	mergesort(lettere, 0, lettere.size()-1);
	...
	mergesort(citta, 0, citta.size()-1);
	if (! is_sorted(citta) ) {

...
{ 
	...
	for (int i=0; i < 100; i++) {
		...
		quicksort(vec, 0, (int)vec.size()-1);		//metto int per evitare che se si genera un vettore di dimensione 0, si vada a ricadere in un -1 che vada in un numero enorme (size_t è unsigned!)
	...
	quicksort(costellazioni, 0, costellazioni.size()-1);
	...
	quicksort(lettere, 0, lettere.size()-1);
	...
	quicksort(citta, 0, citta.size()-1);
	if (! is_sorted(citta) ) {
